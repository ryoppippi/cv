...
#alta(
  ...
  - #styled-link("https://github.com/ryoppippi/curxy")[curxy] (#icon("star")393): Simple proxy worker for using ollama in cursor. If we want to use local LLM models in cursor, we need to have a proxy between local ollama server and remote cursor server. This tool is a simple proxy worker for that.
