...
#alta(
  ...
  [
    ...
    ==== AI Tools
    - #styled-link("https://github.com/ryoppippi/ccusage")[ccusage] (#icon("star")10.9k): CLI tool for analysing Claude Code token usage and costs from local JSONL files. Features include daily/monthly/session-based reports, live monitoring, real-time dashboard, model-specific tracking, and cost analysis. Built with TypeScript and designed for incredibly fast and informative insights.
    ...
    - #styled-link("https://github.com/ryoppippi/curxy")[curxy] (#icon("star")409): Simple proxy worker for using ollama in cursor. If we want to use local LLM models in cursor, we need to have a proxy between local ollama server and remote cursor server. This tool is a simple proxy worker for that.
